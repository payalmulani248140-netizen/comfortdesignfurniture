...
class SliderComponent extends HTMLElement {
  constructor() {
    ...
    this.progressBar = this.querySelector('.slider-progressbar__fill');
    ...
    // Desktop only? Enable this line:
    // this.autoplayEnabled = window.matchMedia('(min-width: 1025px)').matches;

    this.autoplayEnabled = true; // autoplay always ON
  ...
  updateProgressBar() {
    if (!this.progressBar || !this.totalPages) return;
  ...
  addSwipeSupport() {
    ...
    const start = (e) => {
      isDown = true;
      this.stopAutoplay(); // pause autoplay during drag
  ...
  this.autoplay = setInterval(() => {
    let maxScroll = this.slider.scrollWidth - this.slider.clientWidth;

    // If we've reached (or are very close to) the end → reset to first slide
    if (this.slider.scrollLeft + 10 >= maxScroll) {
      this.setSlidePosition(0); // restart from beginning
    } else {
